...
const seed = {
  ...
  schedules: [],
  settings: {showSourceInfo:true,autoSendEnabled:false,smsFallback:true,backendBaseUrl:'',backendApiKey:'',offlineOnly:false}
};
...
function updateOfflineOnlyUI(){const on=!!state.settings.offlineOnly,el=$('#offlineOnlyState');if(el){el.textContent=on?'🔒 Đang khóa kết nối API/backend':'🌐 Đang cho phép kết nối mạng';el.className='mini-pill '+(on?'good':'warn')}const ids=['#testApiBtn','#syncSchedulesBtn'];ids.forEach(id=>{const b=$(id);if(b){b.disabled=on;b.title=on?'Đang bật Chế độ riêng tư':''}})}
